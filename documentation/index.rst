...
- Accepts YouTube clips or URL, phone-recorded city-walk videos, pre-existing datasets, or folders of RGB frames (see :doc:`./source/quickstart/urbanverse_gen` for more details).
- All inputs are normalized into sequential images for scene distillation.

2. **Run UrbanVerse-Gen Pipeline:**

- Generate your own simulation scenes using UrbanVerse-Gen APIs automatically (see :doc:`./source/quickstart/urbanverse_gen` for more details).
...
- Materializes the scene graph by retrieving mutiple matched assets from UrbanVerse-100K (see :doc:`./source/quickstart/urbanverse_100k` for more details), including 3D GLB objects PBR ground materials, and HDRI sky maps.
...
1. **UrbanVerse-160:** A collection of 160 real-to-sim city scenes generated by UrbanVerse-Gen from city-tour YouTube videos acorss the world (See :doc:`./source/quickstart/urbanverse_scenes` for more details).
2. **CraftBench:** A suite of 10 artist-crafted simulation scenes, useful for benchmarking policy robustness and generalization. (see :doc:`./source/quickstart/craftbench` for more details).
...
- **Reinforcement Learning:** Train navigation policies in UrbanVerse scenes using reinforcement learning (PPO) (see :doc:`./source/robot_learning/reinforcement_learning/index` for more details).
- **Expert Data Collection for Imitation Learning:** Collect expert demonstrations from teleoperation (keyboard, joystick, gamepad, VR), and train behavior cloning policies (see :doc:`./source/robot_learning/imitation_learning/index` for more details).
- **Multimodal dataset collection:** Collect offline multimodal dataset (RGB, depth, normals, segmentation, poses, etc) from UrbanVerse scenes for training and evaluation (see :doc:`./source/developer/collecting_data` for more details).
- **Closed-loop evaluation:** Evaluate the performance of the trained policies in UrbanVerse scenes (see :doc:`./source/quickstart/test_your_robots` for more details).
- **Zero-shot Sim2Real deployment:** Deploy the trained policies on real robot platforms (see :doc:`./source/developer/realworld_deployment` for more details).
...
.. toctree::
   :maxdepth: 1
   :caption: User Pipeline Overview

   source/user_side_pipeline/index
